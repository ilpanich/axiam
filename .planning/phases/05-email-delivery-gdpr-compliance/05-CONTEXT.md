...
### Scoped Decision (recorded during planning revision, 2026-06-02)
- **Admin email-config CRUD API is OUT of scope for Phase 5.** REQ-6 "Email provider configurable via settings: SMTP, SendGrid, Postmark, Resend, Brevo" is satisfied this phase by the DB-backed `SurrealEmailConfigRepository` (D-17): provider config is per-org/tenant, secrets are encrypted at rest, the effective provider resolves at send time, and the round-trip test (Plan 05-01) covers all five provider variants. Provider rows are written/seeded **via the repository** for now. An **admin-facing config CRUD API/UI is deferred to follow-up task T19.20** (`claude_dev/roadmap.md`). Rationale: no admin config API was discussed or locked in this phase; the `<domain>` boundary explicitly excludes a "generic template-management UI", and "configurable via settings" is met by DB-backed, per-tenant configuration.
...
- `crates/axiam-server/src/main.rs` — load `AXIAM__EMAIL_ENCRYPTION_KEY` (D-17) + HMAC pepper key (D-02); spawn the purge + export jobs; (if needed) run the email-secret backfill migration (D-17).
