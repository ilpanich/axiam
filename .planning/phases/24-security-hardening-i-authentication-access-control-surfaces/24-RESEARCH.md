...
1. **Does the gRPC governor's key extractor need the same `trusted_hops` fix as REST, or is mesh-internal exposure a sufficient mitigation?**
   - What we know: `SmartIpKeyExtractor` (gRPC) unconditionally trusts the leftmost XFF entry with zero configurability; CLAUDE.md documents gRPC as "for low-latency authz checks in service mesh" (suggesting internal-only exposure); CONTEXT.md's canonical_refs lists the gRPC `rate_limit.rs` file under SECHRD-03's surfaces but D-01c's prose only unambiguously commits to the STORE swap for gRPC.
   - What's unclear: whether the security boundary genuinely excludes external attacker-controlled XFF headers reaching the gRPC service (depends on deployment topology not fully documented in this repo's k8s manifests review scope).
   - Recommendation: planner should either (a) implement a custom gRPC key extractor for parity (low cost, strictly safer) or (b) explicitly document the mesh-internal-exposure assumption in the phase's `<threat_model>` block and get it flagged for the next `/gsd-discuss-phase` or human checkpoint. Do not leave it silently unaddressed either way.
...
2. **Does `confirm_reset`'s current-password-reuse gap (Pitfall 4) get its own explicit fix, or does seeding history + the existing count-based check suffice once ordering is corrected?**
   - What we know: reordering (seed history before evaluating, or add a direct hash comparison) both close the SPECIFIC "reset to your current password" gap the AC names.
   - What's unclear: whether reordering alone (move the `history_repo.create(...)` call before `evaluate_password(...)`) is sufficient, vs. needing an independent `verify_password(new_password, &user.password_hash, ...)` check — reordering has a subtlety: `evaluate_password`'s `check_history` compares against the `password_history_count` MOST RECENT entries; if `password_history_count` is small (policy-configurable) and the user has reset many times, inserting the current hash first still respects the count window, so reordering alone likely suffices for typical config — but a direct comparison is more robust and self-documenting regardless of policy history depth.
   - Recommendation: prefer the direct comparison (also matches the AC's literal wording "blocks reuse of the current password," independent of the history-count setting).
