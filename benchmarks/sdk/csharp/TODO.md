...
`Program.cs` times the four canonical ops (`login`, `refresh`,
...
`../../../../axiam-csharp-sdk/Axiam.Sdk/Axiam.Sdk.csproj` (the release may not be
on NuGet yet). Once `Axiam.Sdk` is published, swap that for the commented-out
`<PackageReference Include="Axiam.Sdk" Version="1.0.0-beta12" />` — and note the
record's `sdk_version` comes from `../_sdkversion.sh` either way, so it keeps
naming the SDK actually built against.
