...
| Resource servers verify the binding — **gRPC** | **landed** (contract 1.17) | The X5 work landed REST-first, and a sweep of the gRPC surface found the omission was not cosmetic: `IntrospectTokenResponse` carried no `cnf`, so a mesh resource server introspecting over gRPC got `active: true` for a bound token and **could not tell it was bound** — forced into exactly the downgrade rule 9 exists to prevent, and unable to satisfy rule 9 detail 4 even in principle. Both gRPC token responses now carry `cnf` and `token_type`; introspection also gains the RFC 7662 §2.2 fields it never had (`scope`, `client_id`) plus `permissions` (X2 UMA) and `ext_exchange_iss` (X4). The interceptor enforces certificate binding against the peer chain, and **refuses** `jkt`-bound tokens because a tonic interceptor sees neither `htm` nor `htu` — the rule-9 answer for a transport that genuinely cannot verify |
