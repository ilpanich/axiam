...
| **SEC-005** SAML XSW / binding gaps (→ SECFIX-04) | ✅ FIXED | Signature bound to the consumed assertion: `bind_signature_to_assertion(xml, &assertion.id)` (`axiam-federation/src/saml.rs:474`), enforcing exactly one `<Assertion>` and a signed `Reference URI=#<id>` (`:792-851`). Authenticated ACS passes real `Destination`/`InResponseTo` (`handlers/federation.rs`, `saml_acs`), not `None`. **Residual closed 2026-08-15 (remediation R1.5):** the bearer `<SubjectConfirmationData>` is now REQUIRED and validated by `validate_bearer_subject_confirmation` (`axiam-federation/src/saml.rs`) — `@Recipient` must equal the SP's real ACS URL (the *same* `expected_destination` argument the `Destination` check reads, so the two can never drift onto separate config paths), `@NotOnOrAfter` must not have passed (one shared `Utc::now()`, same strict no-leeway policy as `<Conditions>`), and `@InResponseTo` must equal the outstanding AuthnRequest id. Fail-closed: a missing `Subject`, a missing bearer `<SubjectConfirmation>`, a bearer confirmation with no `<SubjectConfirmationData>`, or one lacking `@Recipient`/`@NotOnOrAfter` is a rejection. This matters because `Destination` and `Response@InResponseTo` live on the **unsigned** `<samlp:Response>` root and are attacker-rewritable, whereas the SCD is inside the signed assertion. Six new tests + five new signed fixtures (`tests/fixtures/saml/scd_*.xml`); the three negatives (wrong `Recipient`, expired SCD `NotOnOrAfter`, foreign SCD `InResponseTo`) were first run against the unfixed code and confirmed to be **accepted**. No disclosed deferrals remain. Residual (pre-existing, not a regression): the *public* first-time-SSO ACS path (`saml_acs_public`) has no ACS URL to compare against — `saml_login_public` builds its `AuthnRequest` with an empty ACS URL and `FederationLoginState` has no column for one — so there `@Recipient` presence is required but its value is unchecked; closing it needs a schema/config addition. |
...
**Verdict: 14 ✅ / 1 🔶. No regressions.** The remaining compliance-audit deferrals (PERF-01 load test, SBOM) are honestly disclosed in `security-audit.md`, not hidden. The SAML `Recipient` / `SubjectConfirmationData` deferral recorded here is **closed** as of 2026-08-15 (remediation R1.5 — see the SEC-005 row above and `security-audit.md` §7 row SAML-01).
