...
| `reactors` | — | **gap (P1)** | X1's reactor registrations are API-only. This is the highest-consequence gap in the table: a reactor can **veto a login** or **add a claim to a token**, and today the only way to see which ones a tenant has, whether one is failing closed, or whether it has connected at all (`last_seen_at`) is to call the endpoint. `GET /api/v1/reactors/events` exists precisely so a page can render the registry rather than hard-code it. Planned with C4: list + editor + per-reactor health. See §Planned |
...
| Reactor console | X1 | List + editor + per-reactor health. The editor must render the event options from `GET /api/v1/reactors/events` rather than a hard-coded list, so a hook added to the registry appears without a frontend release. Health means three things an operator cannot get from the API without reading it carefully: `last_seen_at` (never-connected is a different state from silent-since), recent timeouts and vetoes from the audit trail, and — the one worth a badge — whether a reactor is `fail_closed`, because that reactor going down takes logins with it |
