...
| `srp` | `LoginPage` (via `services/srp` + `lib/srp`) | covered | §23. The login page probes `/auth/srp/challenge` before falling back to `/auth/login`, so a tenant on `srp_mode: optional` actually gets SRP logins from the browser rather than only from SDK clients. The exchange is real client-side crypto (native `BigInt` + WebCrypto, Argon2id via `hash-wasm`); `M2` is verified and a mismatch aborts sign-in and discards the session rather than retrying over the password path. `403 srp_required` gets its own message so a user is never told their working password is invalid |
| `srp_enrollment` | `profile/ChangePasswordPage`, `auth/ResetPasswordPage`, `BootstrapPage` | covered | §23. Not a page of its own — a verifier is computed and attached wherever a password is set, because the server cannot derive one (it never sees the plaintext). The change-password path reads the tenant's group/KDF from `/auth/me`; the unauthenticated reset path reads it, plus the account's canonical username, from the token-gated `GET /auth/reset/context`. Admin-driven user creation (`users/UsersPage`) does **not** yet attach one: an admin setting another user's initial password would have to know that user's eventual password, which is exactly the flow SRP is meant to remove — such users enrol on their first password change instead |
