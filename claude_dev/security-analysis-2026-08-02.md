...
- **Commits**: server `5edb6b47` (SEC-086 full closure, SEC-087, CHANGELOG), `196ed6b0` (§21 record), plus six dependabot merges. HEAD `50cee941`. SDKs at `origin/main`; the Java SDK carries a security fix of its own (`26debe1`).
- **Result**: **every finding in this document is now closed.** SEC-086 is fully closed on the description axis, SEC-087 is fixed, the CHANGELOG inversion is corrected in both places, and rule-8 guardrail tests exist in all eleven SDKs. What remains is three small residuals, two of them newly created by these fixes, and one accepted timing channel.

### 22.1 Verification

| Item | Verdict |
|---|---|
| **SEC-086 — full closure** | ✅ **CONFIRMED** (ordering verified by hand). `handle_authorization_code` now runs **presence → lookup → verify → grant check** (`token.rs:473-512`), byte-for-byte the same order as the other two grants. The consequence is the one that matters: `unauthorized_client` is now reachable **only after** `verify_client_secret` has succeeded, so neither probe distinguishes existence. §20 went further than §19 asked — it found that after the first fix a **dummy-secret** probe still leaked via `unauthorized_client`, and closed that too. An enumeration of every `error`+`error_description` reachable without valid credentials confirms a single merged terminal (`invalid_client` / "invalid client credentials"), with HTTP status consistent so the code does not re-open the split. The regression test asserts **both** probes across all three client outcomes. |
| **SEC-086 — timing** | 🔶 **Still open, and accurately declared.** No dummy HMAC; an unknown client returns before any hash is computed. The remediation says so plainly rather than claiming a fix. I continue to regard the reasoning as defensible — the DB-miss-versus-row-read delta dominates a sub-microsecond HMAC — and record it as accepted, not missed. |
| **SEC-087** | ✅ **CONFIRMED.** The tenant is resolved via a real repository read **before** the audit write, unknown tenants are dropped with a warning, `attempted_client_id` is truncated at 128, and the recorded IP is now the unforgeable `peer_addr()` with the forwarding header demoted to a `forwarded_for_untrusted` metadata flag. Tests pin all three. **The remaining behaviour is deliberate and I agree with it**: an anonymous caller can still cause rows against a tenant that *exists*, because that event is true and suppressing it would blind an operator to a real attack. Volumetrically bounded by `token_per_min: 120` per IP. |
| **CHANGELOG inversion** | ✅ **CONFIRMED fixed in both places.** `### Changed` now leads with the breaking `axiam:m2m` device audience matching the code, the inverted parenthetical is deleted, and the stale in-code doc comment was corrected too — with an explicit *"this comment previously said…"* note rather than a silent rewrite. |
| **Rule-8 guardrail tests (§21)** | ✅ **CONFIRMED in all eleven** (checked directly). Worth noting the two that needed no new commit are not gaps: TypeScript has a dedicated *"rejects a failed caller token and consults no other credential"* case, and Python takes a structural approach — it signs a healthy token, asserts the verifier never reaches it, **and** asserts the verify signature has no client/session parameter at all, so rule 8 cannot be violated by construction. For a stateless verifier that is a stronger guarantee than a behavioural test. |
| **Dependency bumps** | ✅ **Benign.** The `pem` 3.0.6 → 4.0.0 major is a `base64` + MSRV consequence only — the crate's `src/` is **byte-identical** between the two versions, so no API or parsing-strictness change and no code change required. The added `RUSTSEC-2026-0235` (rkyv) suppression was independently verified as a genuine lockfile-artifact false positive (`cargo tree -i rkyv` finds nothing reachable), and it carries a re-check command and review date — the correct way to suppress an advisory. |

**A correction to my own framing.** I flagged the `pem` bump as touching "the PKI/cert path". That was wrong: `pem` is a dependency of **`axiam-federation` only**, used to convert a stored SAML IdP signing certificate to DER. `axiam-pki` does not depend on it at all — it uses `x509-parser` and `rcgen`. The bump was benign for a different reason than the one I was checking.

### 22.2 The Java SDK found a rule-8 variant of its own

`26debe1` clears `SecurityContextHolder` when a caller's token is rejected. This is a genuine find and it was not on my list: rejecting the request is not sufficient on its own in a servlet container, because an `Authentication` placed in the ambient context by an earlier filter — or left on a pooled container thread — survives the rejection, so a later read sees an identity this caller never authenticated as. That is the **servlet analogue of SEC-085**, reached through thread-local state rather than through a fallback call. It only runs on the rejection path, so credential-free traffic is untouched.

Recording it because of where it came from: rule 8 was written to describe a control-flow mistake, and applying it to a new language surfaced the same mistake in a different mechanism. That is the argument for a written contract doing work no per-SDK review does.

### 22.3 Residuals

1. **Tenant-existence timing differential** *(new, created by the SEC-087 fix)*. The tenant lookup is awaited before the error response, and the audit insert only follows when the tenant exists — so a failed client auth is now measurably faster for a nonexistent tenant than a real one, on an unauthenticated endpoint. Same class as the channel SEC-086 spends its argument on, introduced by the fix for the neighbouring finding. Low: tenant ids are not generally secrets.
2. **Audit telemetry fails open on a lookup error** *(new)*. Any non-`NotFound` error from the tenant read silently drops the failed-auth audit row, logging at `error!` only. The rationale — a DB outage must not turn a 401 into a 500 — is right, but the effect is that a database fault costs exactly the telemetry this row was added to provide, and a database fault is plausibly correlated with an incident.
3. **The 128 cap is chars, not bytes.** A multibyte `client_id` stores up to 512 bytes, and the test asserts a *byte* length that passes only because its payload is ASCII. Cosmetic, and pre-existing in the sibling `client_ip`/`user_agent` helpers, but the test does not check what it appears to check.
4. **Carried**: the gRPC m2m/user parity gap (the interceptor rejects unknown audiences but does not separate the two), and `client_ip`'s docstring overstating that Actix validates trusted proxies — it does not.

### 22.4 Standing status
