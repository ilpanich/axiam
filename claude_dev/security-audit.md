...
| SECFIX-04 | V2 (Authentication) | SAML XML Signature Wrapping closed — signature bound to the consumed assertion; `Destination`/`InResponseTo` validated on the authenticated ACS path. Its one disclosed residual (bearer `<SubjectConfirmationData>`: `@Recipient`/`@NotOnOrAfter`/`@InResponseTo`) closed 2026-08-15 by remediation R1.5 — see §7 row SAML-01 | `.planning/REQUIREMENTS.md#secfix-04-saml-signature-to-assertion-binding` |
...
| SAML-01 | SAML bearer `<SubjectConfirmationData>` (`@Recipient`, `@NotOnOrAfter`, `@InResponseTo`) not validated on the ACS path — SECFIX-04's disclosed residual | Medium | SAML 2.0 Web-Browser-SSO profile §4.1.4.2–3; ASVS V2 (Authentication) | **Fixed (2026-08-15, remediation R1.5)** | SECFIX-04 (origin) — closed by remediation task R1.5, `claude_dev/remediation-plan-2026-08-15.md` | SECFIX-04 bound the XML signature to the consumed assertion and validated `Response@Destination`/`Response@InResponseTo`, but left the bearer `<SubjectConfirmationData>` unchecked (disclosed in `final-security-review.md`, SEC-005 row). Both `Destination` and `Response@InResponseTo` sit on the **unsigned** `<samlp:Response>` root, so an attacker relaying an assertion the same IdP minted for a *different* SP could rewrite them and be accepted. `crates/axiam-federation/src/saml.rs::validate_bearer_subject_confirmation` now requires at least one `Method="urn:oasis:names:tc:SAML:2.0:cm:bearer"` `<SubjectConfirmation>` and enforces, inside the signed assertion: `@Recipient` == the SP's real ACS URL (the same `expected_destination` value the `Destination` check reads — one source of truth, `handlers/federation.rs::saml_acs` → `SamlAcsRequest.acs_url`), `@NotOnOrAfter` not passed (one shared `Utc::now()`, identical strict no-leeway policy as `<Conditions>`), `@InResponseTo` == the outstanding AuthnRequest id. Fail-closed: absent/empty confirmation data is a rejection. Tests: 6 in `saml.rs` over 5 new signed fixtures (`tests/fixtures/saml/scd_*.xml`); the three negatives were confirmed to be *accepted* by the pre-fix code before the fix landed. **Residual (pre-existing, tracked, not beta-blocking):** the public first-time-SSO ACS path (`saml_acs_public`) cannot compare `@Recipient` — `saml_login_public` issues its `AuthnRequest` with an empty ACS URL and `FederationLoginState` stores none — so there presence is required but the value is unchecked; closing it needs a schema/config column. |
...
(SBOM-01) is raised directly by this audit's §3/§4 mapping work. SAML-01 records
SECFIX-04's one disclosed residual and its closure on 2026-08-15 (remediation
R1.5) — it was the last unfixed disclosed security deferral in the AXIAM
document set.
