...
| SEC-029 | MEDIUM — npm vulnerabilities | `npm audit`: **0 vulnerabilities** (axios 1.18.1, react-router 8.3.0, vite 8.1.5); CI gates `npm audit --audit-level=high`. Dependabot GHSA-qwww-vcr4-c8h2 (react-router RSC CSRF bypass, `>=7.12.0 <8.3.0`) cleared by migrating `react-router-dom@7.18.1` → `react-router@8.3.0`; the SPA never used the affected unstable RSC APIs, so it was not exploitable, but the dependency is now off the vulnerable range. |
