...
*47 threats — 3 critical, 22 high, 20 medium, 2 low; 0 open.*
...
| T-39 | Medium | Access token still valid after entitlement revocation | Token service EdDSA JWT + refresh rotation <br/>*Authentication & session management* | Narrowed server-side on 2026-09-12: an optional, off-by-default `GET /oauth2/revocations` publishes hashed ids of sessions revoked within the last token lifetime, so a guard that polls it rejects within one poll interval. Open because **no SDK polls it yet** — a feed nobody reads narrows nothing (contract 1.44 §10.4.1 is the per-SDK table)… |
...
| T-143 | Medium | Local JWT verification misses a revoked entitlement | SDK token verification (JWKS cache, iss/aud) <br/>*Client SDKs & admin UI integration surface* | Bounded by the 15-minute access-token lifetime. Contract 1.44 §10.4 now defines the optional revocation-feed poller that would narrow it to one poll interval — default off, bounded, never fail-closed. Open until an SDK implements it… |
...
- **Access tokens survive revocation for up to 15 minutes.** The price of stateless verification. Use gRPC introspection where immediate revocation matters — or, since 2026-09-12, turn on the revocation feed (`AXIAM__AUTH__REVOCATION_FEED_ENABLED`) and use an SDK guard that polls it, which narrows the window to one poll interval for the cost of one cacheable fetch per interval rather than a round trip per request. Off by default on both sides, never fail-closed, and it narrows the trade rather than removing it (T-39, T-143).
