...
| T-272 | /oauth2/register (RFC 7591, unauthenticated) <br/>*Process* | D | A stranger fills the tenant's registration quota and denies registration to legitimate clients until the sweeper runs | Medium | **Open** |
...
| T-276 | Client ID metadata document fetch <br/>*Process* | I | The trusted-publisher list that bounds the fetch admits a value meaning "every host" | Medium | **Open** |
| T-275 | externally registered clients (dcr, cimd) <br/>*Store* | D | Shadow client rows accumulate without a quota and are reclaimed by nothing | Medium | **Open** |
...
| T-280 | /oauth2/authorize (+ consent) <br/>*Process* | D | A desktop client on an ephemeral loopback port is never told its authorization failed | Low | **Open** |
...
`/oauth2/register (RFC 7591, unauthenticated)` (Process) · Denial of service · Medium · **Open**
...
> Bounded, not closed. `initial_access_token` mode is unaffected: a caller holding no handle is refused before the quota is consulted, which is a real reason to prefer that mode and one the operator page does not currently give. `anonymous` mode is opt-in, refused while `external_client_allowed_resources` is empty, and every attempt is rate-limited and audited, so the denial is noisy and attributable to whatever addresses it came from. **Open** because nothing today shortens the window: the fix is to sweep a registration that was never authorized on a much shorter clock than one that was, which turns a month of denial into an hour and needs a settings field to be configurable. Filed as MCP-05 ([#471](https://github.com/ilpanich/axiam/issues/471)) against T21.4a; the same section records that the quota check and the write are separated by an `await`, so concurrent registrations can also overshoot the ceiling by the number in flight.
...
`Client ID metadata document fetch` (Process) · Information disclosure · Medium · **Open**
...
> Not a default: CIMD is off by default and the field ships empty, so no deployment has this posture without an operator writing it. The address guard still holds, so the residual is an outbound `GET` to public URLs a caller chooses, with AXIAM's source address and no attribution — not an internal-network primitive. **Open**, filed as MCP-03 ([#469](https://github.com/ilpanich/axiam/issues/469)) against T21.5: the fix is to refuse `*` in `trusted_client_id_domains` specifically, leaving it admissible in `trusted_redirect_domains` where empty is a working posture and the entries are not fetch targets. One condition and one message in the validator that already holds the empty-list refusal; it is not made here because the message needs drafting against the operator page.

**T-275 — Shadow client rows accumulate without a quota and are reclaimed by nothing**  
`externally registered clients (dcr, cimd)` (Store) · Denial of service · Medium · **Open**
...
> The bound is the number of distinct URLs that both match `trusted_client_id_domains` and serve a valid document, which for the profile the documentation recommends — a named publisher, `*.vendor.example` — is small, and the entry would be theoretical. It stops being theoretical the moment a tenant trusts shared hosting, which is a natural thing to do because shared hosting is where a small tool publishes a JSON file; any third party who can publish under that domain then mints unbounded rows at one unauthenticated request each. It compounds with T-276, where `*` makes every domain shared hosting. **Open**, filed as MCP-04 ([#470](https://github.com/ilpanich/axiam/issues/470)) against T21.5. The cheap half of the fix is a quota counting `Cimd` beside `Dcr`; the complete half is eviction on last-seen, which is *coherent* with the cache argument rather than against it — a row deleted while its document is still published is re-materialised on the next request, which is the behaviour a cache should have.
...
`/oauth2/authorize (+ consent)` (Process) · Denial of service · Low · **Open**
...
> Fail-closed in the direction that matters: no error is ever redirected to a URI that was not registered, so this is not an open-redirect finding and the exact comparison is the safe error to make. What it costs is interoperability, on exactly the client family Phase 21 exists to serve. Confined to refusals raised *before* the matcher runs — `response_type` absent entirely, and the `request_uri` refusals — which the 2026-09-17 review established by driving both the negative and the positive case after its first, wider, framing of the finding was contradicted by the harness. **Open**, filed as MCP-01 ([#472](https://github.com/ilpanich/axiam/issues/472)) against T21.2a: the fix is to route the six sites through `any_redirect_uri_matches`, which already answers the question for the success path, and is mechanical.
...
17 of 280 threats remain open, and four of them are new and are a different kind from the other thirteen. T-272, T-275, T-276 and T-280 arrived with Phase 21 from [`security-review-mcp-2026-09-17.md`](security-review-mcp-2026-09-17.md) and **do** sit on AXIAM's own request path: they are filed defects with named fixes, not trade-offs, and they are open because each needs a settings field, a migration or a change to a handler its own task did not touch. They are bounded rather than unhandled — three of the four are unreachable on default settings, and the fourth is fail-closed — but a reader should not file them mentally beside the deployment responsibilities below. The other thirteen are unchanged: none of them is an unhandled defect in AXIAM's own request path: they are accepted design trade-offs, responsibilities that land on whoever deploys AXIAM, or gaps on the SDK and distribution side. The one entry that did sit on the request path, T-254's refresh-rotation grace window, was closed by the maintainer's decision of 2026-09-12 and is no longer listed here; the two that sat on the token service and on the SDK guard since the first version of this model, T-39 and T-143, closed together on 2026-09-13 when the revocation feed gained a poller in every SDK, and are recorded under *Closed at 1.0.0-beta14* below. They are listed most severe first.
...
| T-272 | Medium | A stranger fills the tenant's registration quota and denies registration to legitimate clients | /oauth2/register (RFC 7591, unauthenticated) <br/>*OAuth2 / OIDC authorization server* | Filed as MCP-05, [#471](https://github.com/ilpanich/axiam/issues/471) (T21.4a). `anonymous` mode only, which is opt-in and refused while `external_client_allowed_resources` is empty; `initial_access_token` mode is unaffected. Open because one TTL serves both "registered and abandoned" and "used once, gone quiet", so a never-authorized row sits for 30 days by default. The fix — a much shorter clock for a registration that was never authorized — needs a settings field. |
| T-275 | Medium | Shadow client rows accumulate without a quota and are reclaimed by nothing | externally registered clients (dcr, cimd) <br/>*OAuth2 / OIDC authorization server* | Filed as MCP-04, [#470](https://github.com/ilpanich/axiam/issues/470) (T21.5). `cimd` rows count against no quota and the sweeper excludes them by design. Bounded by `trusted_client_id_domains` and therefore theoretical for a named publisher; not theoretical for a tenant that trusts shared hosting. Needs a schema field, a migration and a job registration to close. |
| T-276 | Medium | The trusted-publisher list that bounds the CIMD fetch admits a value meaning "every host" | Client ID metadata document fetch <br/>*OAuth2 / OIDC authorization server* | Filed as MCP-03, [#469](https://github.com/ilpanich/axiam/issues/469) (T21.5). The interlock refuses the empty list and accepts `*`, which means the same thing, and the validator's own error message names `*` as valid. Not a default. The SSRF address guard still holds, so the residual is public-internet request forgery rather than an internal-network primitive. |
| T-280 | Low | A desktop client on an ephemeral loopback port is never told its authorization failed | /oauth2/authorize (+ consent) <br/>*OAuth2 / OIDC authorization server* | Filed as MCP-01, [#472](https://github.com/ilpanich/axiam/issues/472) (T21.2a). Fail-closed — no error is redirected to an unregistered URI — so this is interoperability, not an open redirect. Confined to refusals raised before the redirect matcher runs. The fix routes six call sites through `any_redirect_uri_matches` and is mechanical; open because those sites are in a handler T21.8 does not otherwise touch. |
