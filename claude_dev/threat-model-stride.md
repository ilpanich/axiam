...
> Refresh tokens are opaque, server-stored and single-use with rotation; redeeming a token that has already been rotated is detectable and invalidates the family. **Amended at 1.0.0-beta13:** rotation now *supersedes* rather than revokes — the previous token stays redeemable for a 60-second grace (FAPI 2.0 §5.3.2.1-9) before its brought-forward expiry retires it — so "detectable" holds only outside that window; T-254 records the residual, and records it open.
...
| T-254 | /oauth2/token (code, refresh, client credentials) <br/>*Process* | S | A leaked refresh token replayed inside the rotation grace window forks the session undetected | Medium | Open |
...
`/oauth2/token (code, refresh, client credentials)` (Process) · Spoofing · Medium · Open

FAPI 2.0 Security Profile §5.3.2.1-9 requires an authorization server that rotates refresh tokens to keep accepting the previous one for a period after issuing its successor — the only recovery a client has from a rotation response lost in transit, since under immediate revocation it holds a token the server has destroyed and has not been given the replacement. AXIAM now **supersedes** rather than revokes: the old token's `expires_at` is brought forward to a 60-second grace instant (`REFRESH_ROTATION_GRACE_SECS`), for every client on every profile. Inside that window a second presentation is answered `200` and rotates again, so a refresh token leaked at the moment of a legitimate rotation yields a second live chain, and nothing distinguishes the thief's use from the honest retry. Before 065f37c the second use was refused outright, which is what T-37 records.

> Accepted design trade-off, bounded and stated rather than made silently. The window is 60 seconds and can only ever be brought forward — `expires_at < $grace_until` in the SET, so a caller passing a distant instant lengthens nothing — and after it the token is expired exactly as before, answered "already consumed". `revoked = false` in the WHERE keeps two genuinely concurrent rotations from both finding a live row, so the loser still gets `NotFound` and the single-use race T-37 closes stays closed. A password or MFA reset revokes the whole family through `revoke_all_for_user`, and T-249's `sid` extends that to the access tokens in flight. On the `fapi2` profile — the one that requires the grace — every token is sender-constrained, so replaying one inside the window needs the client's private key as well, which is why the profile that requires the window is also the one that can afford it. **Open because on the `standard` profile a bearer refresh token now has a 60-second replay window it did not have, and the server cannot tell the thief from the retry.** Closing it is a decision, not a fix: either reuse detection inside the window — revoke the family when a superseded token is presented *after* its successor has itself been used — or confining the grace to sender-constrained clients. `refresh_token_rotation_retires_old_on_the_grace_clock` pins the bound (thirty days becoming sixty seconds), which is the property that survives the change.
...
*25 threats — 7 critical, 14 high, 4 medium; 1 open.*
...
| T-254 | Medium | A leaked refresh token replayed inside the rotation grace window forks the session undetected | /oauth2/token (code, refresh, client credentials) <br/>*OAuth2 / OIDC authorization server* | Accepted design trade-off, bounded: FAPI 2.0 §5.3.2.1-9 requires the previous refresh token to stay usable briefly after rotation, and since 1.0.0-beta13 AXIAM applies a 60-second grace to every profile. On `fapi2` every token is sender-constrained, so a replay inside the window needs the client's key; on `standard` a bearer refresh token now has a window the server cannot distinguish from an honest retry. Closing it is a decision — in-window reuse detection, or grace only for sender-constrained clients — and is recorded here rather than made silently… |
...
- **A rotated refresh token stays redeemable for 60 seconds** (T-254, since 1.0.0-beta13). FAPI 2.0 §5.3.2.1-9 requires it — it is the only recovery a client has from a rotation response lost in transit — and the profile that requires it sender-constrains every token, so a replay inside the window needs the client's private key. AXIAM applies the same grace on the `standard` profile, where a bearer refresh token gains a 60-second replay window it did not have and the server cannot tell the thief from the honest retry. Two ways to close it, both decisions rather than fixes: detect reuse *inside* the window (revoke the family when a superseded token is presented after its successor has itself been used), or confine the grace to sender-constrained clients. Until one is taken the item stays open; a password or MFA reset still revokes the whole family.
...
- A fix changes what a grant, a policy or a credential *means* even when no surface moves (the beta09 authorization-reach fixes T-226…T-228 and the WebAuthn user-verification policy T-229…T-230 are the examples: nothing new was exposed, but what existing data authorises changed) — and the reverse case, a fix that *weakens* a property the model records, which is written down as an open item rather than absorbed: the beta13 refresh-rotation grace window amended T-37 and opened T-254
