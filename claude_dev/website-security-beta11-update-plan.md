...
> Dragon model is at **2.11.0 — 236 threats, 220 mitigated / 16 open**; the
...
| beta11 | gRPC may be published — only through the edge, path-matched, as a service allowlist; `AXIAM__GRPC_TLS_*`; `STRICT_REVOCATION`; at beta11 the tonic listener had no certificate reload (closed since by R-1 below) | T-233, T-234 |
...
| unreleased, after beta11 ([`remediation-plan-2026-09-04.md`](remediation-plan-2026-09-04.md), all landed on `main`) | **R-1** the gRPC listener terminates its own TLS 1.3 from the *same* reloadable leaf as REST — one `SIGHUP` renews both, no deploy-hook restart; **R-3** the deployment-origin rule on all four SSO start paths (contract 1.39; an off-origin SPA needs `AXIAM__AUTH__SSO_SPA_ORIGINS` on OIDC and OAuth2 too); **R-4** `axiam_rate_limit_xff_discarded_total{protocol}` and a once-per-process `WARN` when `TRUSTED_HOPS` discards the header; **R-5** `ReactorAdminService` in the `Admin` gRPC family (10/s per IP); **R-6** `/health/jobs` in `openapi.json` (not §27 surface); **R-7** `just vault-status` reports the seal type and flags Shamir; **R-2** the §27 drift-check gates every SDK release | T-234 **closed**; T-212, T-213, T-214, T-216, T-219, T-233, T-235 amended |
...
T-216; T-234 was opened by the wave and closed again by the remediation pass, so
the register is back to 16. AXIAM's own request path still carries **no open
Critical or High** finding — keep that sentence, it is still true.
...
- **Do not upgrade the hedges.** "No open Critical or High finding *in AXIAM's
  ...
  caution are load-bearing. Nothing open sits on AXIAM's own request path — the
  one open High, T-216, is a deployment responsibility — so the sentence stands.
...
| Generated model | `website/src/threatModel.ts`, `threatModelSummary.ts` | 211 threats / 196 / 15, model 2.10.0 — `npm run gen:threat-model` was run against 2.11.0 in this pass to confirm it parses and yields `9 diagrams, 236 threats (220 mitigated, 16 open)`, then **reverted** so the generated files and the prose move together |
...
| Contract anchors | `website/src/contractAnchors.ts` | `CONTRACT_VERSION = "1.38"` — behind `sdks/CONTRACT.md`, which moved to **1.39** with R-3 (rule 12a widened to all four SSO start operations); `gen:contract-anchors` will produce a diff |
| API index | `website/src/apiIndex.ts` | Behind `sdks/openapi.json`, which gained `WebauthnPolicy` and `webauthn_user_verification` on the settings schemas at beta09 (`8ae510d`) and `GET /health/jobs` under the `health` tag with R-6 |
...
```sh
npm run gen:threat-model     # expect: threatModel.ts: 9 diagrams, 236 threats (220 mitigated, 16 open)
npm run gen:api-index        # expect: the WebAuthn policy field on the settings schemas, and /health/jobs under the health tag
npm run gen:contract-anchors # expect: CONTRACT_VERSION 1.38 → 1.39 and the §12.1 anchors
```
...
`GET /health/jobs` entered `sdks/openapi.json` with R-6 (it is deliberately not a
§27 SDK operation), so the `observability` and `troubleshooting` pages that tell
operators to alert on it can now link the generated API reference.
...
| The threat model | Table: 236 threats, 220 / 16. The coverage tables are generated — confirm the page renders 236 / 17 from `threatModelSummary.ts`, and that the "Coverage by area" rows match §Appendix A |
...
| Shared responsibility | "16 of 236". The register is generated — confirm T-118 and T-234 are gone and T-216 appears. The **Run Vault in production mode** bullet's "read-only token scoped to that path" becomes the policy-file sentence. Four new platform bullets before **Run SurrealDB on a persistent storage engine**: auto-unseal; deriving `TRUSTED_HOPS`; stripping `X-Forwarded-For` / `X-Client-Certificate`; gRPC only through the edge |
...
| `federation` | A new section, **Sign-in buttons and the public login surface**: the unauthenticated providers listing (`200 []` for unknown and unconfigured alike — an organization-slug oracle is deliberately not offered); the `oauth2` protocol for providers that issue no ID token, its stated downgrade, the provider kinds it is refused for, mandatory server-side PKCE, endpoints only from configuration, verified email only; handoff codes for the cross-site returns (SAML, Apple) — 60 s, single use, hash-only, confined to the deployment's own origin plus `AXIAM__AUTH__SSO_SPA_ORIGINS`; organization→tenant inheritance (user provisioned in the *requesting* tenant; a tenant's own config of the same kind shadows the inherited one, including a disabled one); templated issuers require `allowed_issuer_tenants`; custom icons raster-only, 16 KiB. Endpoints: the four contract §12.1 operations. State that the origin rule applies to **all four** start operations since R-3 (contract 1.39), so an SPA on a different origin needs `AXIAM__AUTH__SSO_SPA_ORIGINS` for OIDC and OAuth2 sign-in as well. Link `docs/` and contract §12.1 rather than restating rules | `claude_dev/federation-sso-login-design.md`; `sdks/CONTRACT.md` §12.1 (1.37, 1.38, 1.39); T-218…T-225 |
...
| `observability`, `hardening`, `secrets` | The remediation pass's operator-visible additions: the counter `axiam_rate_limit_xff_discarded_total{protocol="rest"\|"grpc"}` and the once-per-process `WARN` (R-4) — a rising counter means every client is keying on the proxy, and the rule is proxies − 1; `ReactorAdminService` now sits in the `Admin` gRPC family at 10/s per IP, so reactor CRUD over gRPC above that needs `AXIAM__GRPC__GRPC_ADMIN_PER_SEC` (R-5); `just vault-status` prints the seal type, flags `shamir` as no auto-unseal, and `--strict` fails on it (R-7) | `docs/deployment/README.md`, `docs/deployment/rate-limit-sizing.md` §3.1, `docs/api/grpc.md`, `docs/deployment/vault.md` §5.3; CHANGELOG `[Unreleased]` |
...
- **News.** Do not rewrite "AXIAM reaches beta". Give it a dated addendum
  ...
  236 threats, 220 mitigated / 16 open; the four releases since — the backend on
...
- Server and SDK-CI work: none remains from this wave. R-1…R-7 of
  `remediation-plan-2026-09-04.md` landed on `main` before this pass runs; the
  website describes them, it does not implement them. The one open High, T-216
  (auto-unseal), is a deployment responsibility and stays in the register.
...
Headline: **236 threats, 220 mitigated / 16 open**, 9 diagrams, `threatTop` 236
(model 2.11.0 as amended by the remediation pass; T-234 closed).
...
Critical 28 (1 open), High 113 (8), Medium 88 (6), Low 7 (1). The open items are
T-148, T-18, T-94, T-124, T-133, T-135, T-146, T-216, T-180, T-9, T-39, T-110,
T-123, T-134, T-143, T-161. Every one of these numbers is emitted by the
