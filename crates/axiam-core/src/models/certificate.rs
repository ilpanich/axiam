...
///
/// The leaf counterpart to [`SignIntermediateCsr`], and the BYOK counterpart to
/// [`CreateCertificate`]: the private key is generated by whoever made the CSR
/// — an offline ceremony, an HSM, a device's secure element, a tenant-side tool
/// — and never reaches AXIAM at all.
///
/// There is no `key_algorithm` field, because the key is the caller's and the
/// CSR already says what it is; AXIAM reads it from the request's public key
/// and refuses anything outside its policy rather than recording what it was
/// told. There is no `subject` field for the same reason: the CSR's common name
/// is what the certificate says, so the row and the certificate cannot
/// disagree.
///
/// The response is a plain [`Certificate`] and **not** [`GeneratedCertificate`]:
/// there is no key to return, and a type with a mandatory key field that is
/// always absent is a type that lies.
#[derive(Clone, Serialize, Deserialize, utoipa::ToSchema)]
pub struct SignCertificateCsr {
    /// Taken from the authenticated context, never from the body.
    #[serde(default)]
    pub tenant_id: Uuid,
    /// The signing CA that issues it. Must be Active, in its validity window,
    /// and hold a key AXIAM can sign with.
    pub issuer_ca_id: Uuid,
    /// PEM-encoded PKCS#10 certificate signing request — a
    /// `BEGIN CERTIFICATE REQUEST` block. The legacy OpenSSL
    /// `BEGIN NEW CERTIFICATE REQUEST` header is not accepted.
    pub csr_pem: String,
    pub cert_type: CertificateType,
    /// Validity duration in days. Subject to the same tenant cap and the same
    /// CA/Browser Forum hard cap as [`CreateCertificate`], and capped to the
    /// issuer's own expiry.
    pub validity_days: u32,
    pub metadata: Option<serde_json::Value>,
}
...
/// `Debug` gives: a CSR is public by construction — it carries a public key and
/// a signature proving possession of the private one — but it is bulky and
/// appears in handler-level tracing spans.
impl std::fmt::Debug for SignCertificateCsr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SignCertificateCsr")
            .field("tenant_id", &self.tenant_id)
            .field("issuer_ca_id", &self.issuer_ca_id)
            .field("csr_pem", &format_args!("[{} bytes]", self.csr_pem.len()))
