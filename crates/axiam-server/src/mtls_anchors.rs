...
//! There is no supported way to add a root to a listening server, so flagging a
//! CA changes what the *next* boot trusts. The API says so in its response
...
fn bundle_path(tls: &TlsConfig) -> Option<PathBuf> {
