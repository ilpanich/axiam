...
| 148 | An authorization module can be completed. The W3 login hop redirects to a **same-origin** `/login` (`LOGIN_PATH`), which the `axiam-server` binary does not serve — only the admin SPA does. A conformance target must therefore serve the SPA and the API on one origin | plan §4.0 | Pass | `conformance/nginx-axiam.conf`; `/oauth2/authorize` with a resolved tenant answers `302 /login?return_to=…&axiam_login_hop=1` |
| 149 | The front door and the mutual-TLS listener are **different origins by necessity**, not by preference. `axiam_oauth2::mtls` refuses a forwarded `X-Client-Certificate` for OAuth2 client authentication by construction, so a proxy in front of the token endpoint would make `tls_client_auth` unauthenticatable. The split is expressed through RFC 8705 §5 `mtls_endpoint_aliases` | RFC 8705 §5 | Pass | `conformance/nginx-axiam.conf`; `AXIAM__AUTH__OAUTH2_MTLS_BASE_URL`; `crates/axiam-oauth2/src/mtls.rs` module docs |
| 150 | `code_challenge_methods_supported` is published, and is exactly `["S256"]` — the authorization endpoint refuses `plain`. RFC 8414 defines no default, so its absence read as "PKCE unsupported" | RFC 8414 §2; RFC 7636 §4.3 | Pass | `oidc.rs::discovery_advertises_pkce_and_assertion_signing_algorithms` |
| 151 | `token_endpoint_auth_signing_alg_values_supported` is published, derived from `jose::permitted_algorithm_names()` so the advertisement cannot drift from the verifier | RFC 8414 §2 | Pass | `oidc.rs::discovery_advertises_pkce_and_assertion_signing_algorithms`; `jose.rs::every_permitted_algorithm_has_a_wire_name` |
