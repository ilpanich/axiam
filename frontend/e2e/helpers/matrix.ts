...
export interface NavDestination {
   ...
   * `require_organization_principal`, which keys on where the caller's record
   * lives. Recorded here so the matrix asserts the gate the UI actually
   * applies: a model that knew only about permissions read the fix as a
   * regression and demanded the bug back.
   */
...
export async function canActOnOrganization(page: Page): Promise<boolean> {
  ...
  return page.evaluate(async () => {
    ...
    const body = (await res.json()) as {
      user?: { organization_level?: boolean; reachable_tenant_ids?: unknown };
    };
    // The same two fields, in the same order, as `useCanActOnOrganization` in
    // `src/lib/grantReach.ts`: organization-level, and not narrowed to
    // particular tenants. Read from the endpoint rather than hard-coded per
    // principal so the oracle stays the server.
    if (body.user?.organization_level !== true) return false;
    return !Array.isArray(body.user.reachable_tenant_ids);
  });
}
