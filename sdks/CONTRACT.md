...
3. **A registration may be refused with `503` when the server's reactor
   transport cannot dispatch** (SEC-101). While the deployment composes a
   transport that fails every round trip — which is the case for every build
   until the lapin transport merges (§22.1's scope note) — `POST
   /api/v1/reactors` and any `PUT` that would leave the registration
   **enabled** answer `503 service_unavailable` with a body naming the reason.
   Accepting the registration instead would hand a tenant admin a complete,
   self-inflicted login outage through a supported admin action: the transport
   fails, `login.post_auth` defaults to `fail_closed`, and the first login
   after the registration is denied.
