...
- [`axiam-rust-sdk#{{PR:rust}}`](https://github.com/ilpanich/axiam-rust-sdk/pull/{{PR:rust}})
...
- [`axiam-cplusplus-sdk#{{PR:cplusplus}}`](https://github.com/ilpanich/axiam-cplusplus-sdk/pull/{{PR:cplusplus}})
...
| R-12 | The device credential was never released. A later `login` was shadowed by it, and in Go, C++ and PHP `logout` did not clear it either | Python, Kotlin, Go, Swift, C++, PHP, TypeScript, Rust, Java | §6.1 rule 11.4 | **contract fixed** and **SDK fixed** | N4.4 · Python #90, Kotlin #{{PR:kotlin}}, Go #88, Swift #67, C++ #{{PR:cplusplus}}, PHP #75, TypeScript #119, Rust #{{PR:rust}}, Java #103 |
| R-13 | A `401` on the device credential reached a refresh. PHP did this on REST and gRPC. TypeScript did it on re-authentication (the device path was missing from `SKIP_REFRESH`) and on gRPC. Java and Python did it on gRPC | PHP, TypeScript, Java, Python | §6.1 rule 11.5 | **contract fixed** and **SDK fixed** | N4.5 · PHP #75, TypeScript #119, Java #103, Python #90 |
...
| R-15 | Every management call was refused client-side after a device login, because the session check read only the cookie | Python | §27.4 rule 1 | **contract fixed** and **SDK fixed** | N4.7 · Python #90 |
...
| R-18 | The header, `X-Tenant-ID` or a credential sent to an off-origin host, such as a discovered `/oauth2` endpoint | Python, C# | §5.2 rule 1 | **SDK fixed** | N5.1 · Python #90, C# #{{PR:csharp}} |
| R-19 | `X-Tenant-ID` missing on login, `verify_mfa`, the password-reset context and the WebAuthn setup pair | Rust | §5 rule 2 | **SDK fixed** | Rust #{{PR:rust}} |
...
| R-23 | Which responses record the gate. Rust reset it on OPAQUE finish, MFA setup confirm and WebAuthn setup finish, whose `200` carries the user object | Rust | §5.2 rule 1 | **contract fixed** and **SDK fixed** | N5.5 names the record and reset sets · Rust #{{PR:rust}} |
...
| R-29 | A stated `inherit: true` was refused (Java). `inherit: false` without a resource was sent (C++, TypeScript) | Java, C++, TypeScript | §27.6.1 | **contract fixed** and **SDK fixed** | N6.2 · Java #103, C++ #{{PR:cplusplus}}, TypeScript #119 |
| R-30 | The outcome of a failed rebind's restore was only in a message (Python), or was swallowed (C++) | Python, C++ | §27.6.1 | **contract fixed** and **SDK fixed** | N6.3, "as data" · Python #89, C++ #67 |
