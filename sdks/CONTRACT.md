...
4. **Both KDFs MUST be supported for login.** `argon2id` and `pbkdf2_sha256`.
   An SDK that cannot perform the KDF the server names MUST raise the §2
   `ConfigurationError` taxonomy type with the KDF name in the message — never
   substitute the other one, which would silently derive a wrong `x` and surface
   as "invalid password". Parameters arrive per exchange and MUST be honoured as
   given; an SDK MUST NOT cache them across logins, because a verifier enrolled
   under different costs is still valid and must keep working.
...
10. **`srp_required` is not an authentication failure.** A `403` whose body
    ...
    the credentials were wrong. An SDK's `login` MUST raise a distinguishable
    error (§2 `ConfigurationError`, message naming SRP) so an application can
    switch to `login_srp` and so a user is never shown "invalid username or
    password" for a password that is perfectly good.
