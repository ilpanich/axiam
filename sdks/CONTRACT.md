...
| R-19 | `X-Tenant-ID` missing on login, `verify_mfa`, the OPAQUE start, finish and registration calls, MFA-setup enrolment, the password-reset context, the WebAuthn authentication start and the WebAuthn setup pair | Rust | §5 rule 2 | **SDK fixed** | Rust #{{PR:rust}} |
...
| R-29 | A stated `inherit: true` was refused (Java). `inherit: false` without a resource was sent (C++, TypeScript, C#) | Java, C++, TypeScript, C# | §27.6.1 | **contract fixed** and **SDK fixed** | N6.2 · Java #103, C++ #68, TypeScript #119, C# #{{PR:csharp}} |
| R-30 | The outcome of a failed rebind's restore was only in a message (Python), or was swallowed (C++). C# reported whether the restore succeeded but kept the restore's own error only in the message | Python, C++, C# | §27.6.1 | **contract fixed** and **SDK fixed** | N6.3, "as data" · Python #89, C++ #67, C# #{{PR:csharp}} |
