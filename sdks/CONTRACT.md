...
4. **Self-service endpoints ignore the header, and an SDK MUST NOT help them** (contract
   ...
   `X-Tenant-ID` names. That covers `GET`/`PUT /users/{own id}`, `GET`/`DELETE` on that
   ...
   NOT work around that by clearing or rewriting `X-Tenant-ID` for those calls: the
   ...
   to fix reading its own. Send the header as normal on every request; the server decides.
