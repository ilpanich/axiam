...
const responseJson = assertion.toJSON();   // → back to the SDK, unchanged`,
...

  {
    ...
    blocks: [
      ...
      {
        type: "note",
        text: "Client-certificate verification is native: `AXIAM__SERVER__TLS__CLIENT_AUTH` (`off` | `optional` | `required` | `optional_self_signed`) with a trust bundle at `AXIAM__SERVER__TLS__CLIENT_CA_PATH`. No proxy-asserted identity header is in the trusted path, and a misconfigured mTLS server fails fast at startup rather than serving without verification. `optional_self_signed` additionally admits a certificate that chains to nothing, which the OAuth2 `self_signed_tls_client_auth` method needs — and **only** that: a self-asserted certificate is never a device identity, whatever policy the listener runs.",
      },
