    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "**The cookie is a fourth cookie**, beside the three that already exist and change in no way: `HttpOnly`, `Path=/oauth2/authorize`, `SameSite=Lax` and `Secure` **unconditionally** — it is the only `SameSite=Lax` cookie AXIAM sets, so unlike the other three it does not follow `AXIAM__AUTH__COOKIE_SECURE`. Only its SHA-256 is stored. `Lax` is the point: it travels on a top-level navigation and not inside a frame, so cross-site hidden-iframe silent renew does not work and **fails closed** — which is the same property that defeats cross-site login-status probing. Relying parties renew with a top-level `prompt=none` navigation, or with a refresh token.",
