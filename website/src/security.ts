    ...
    blocks: [
      ...
      { type: "h", id: "platform-ops", text: "Platform & operations" },
      {
        ...
        items: [
          ...
          "**Publish gRPC only through the edge, as an allowlist, or not at all.** A port forwarded straight at the listener lets a client mint its own rate-limit bucket per call, and no setting repairs that. If you enable the gRPC listener's TLS, add the container restart to your certificate deploy hook: unlike the REST listener, it reads its certificate once.",
